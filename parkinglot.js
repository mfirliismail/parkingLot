...
window.addEventListener("DOMContentLoaded", (event) => {
  ...
  const countAvailablePlaces = (event) => {
    document.querySelector("#placesCount").innerHTML =
      totalPlaces - cars.length;
  };

  const setClassForBadge = () => {
    let badgeClassName =
      cars.length == totalPlaces ? "badge badge-danger" : "badge badge-success";
    document
      .querySelector("#placesCount")
      .setAttribute("class", badgeClassName);
  };

  const calculateHoursBilled = (car) => {
    let arrivedAt = new Date(car.arrival).getTime();
    let leftAt = new Date(car.leave).getTime();
    console.log(arrivedAt, leftAt);
    let result = leftAt - arrivedAt;
    console.log(result);
